...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $stmt = $conn->prepare("INSERT INTO purchase (customer_id, product_id, quantity, total_price) VALUES (?, ?, ?, ?)");
    $stmt->bind_param("iiid", $_POST['customer_id'], $product_id, $quantity, $total_price);
    $stmt->execute();
    // ลด stock สินค้า
...
<title>เพิ่มรายการสั่งซื้อ - PHP CRUD</title>
...
<div class="card">
  ...
  <form method="POST" novalidate>
    ...
    <div class="mb-3">
      <label for="productSelect" class="form-label">สินค้า <span class="text-warning">*</span></label>
      <select id="productSelect" name="product_id" class="form-select" required>
        ...
        ?>
          <option value="<?= htmlspecialchars($p['product_id']) ?>" data-price="<?= htmlspecialchars($p['price']) ?>">
            <?= htmlspecialchars($p['product_name']) ?> (<?= number_format($p['price'], 2) ?> บาท)
          </option>
        <?php endwhile; ?>
      </select>
    </div>
    ...
    <div class="mb-3">
      <label for="quantityInput" class="form-label">จำนวน <span class="text-warning">*</span></label>
      <input id="quantityInput" name="quantity" type="number" min="1" value="1" class="form-control" required />
    </div>
    ...
    <button type="submit" class="btn btn-warning w-100">บันทึกรายการ</button>
  </form>
</div>
